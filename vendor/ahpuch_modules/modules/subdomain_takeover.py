...
def banner():
    ...
    =============================================
        Ah-Puch - Advanced Subdomain Takeover Detection
    =============================================
    """)
...
def resolve_subdomains(domain):
    ...
    try:
        ...
        return []
    except (OSError, requests.RequestException, TypeError, ValueError, KeyError):
...
async def check_subdomain_takeover(session, subdomain, semaphore):
    async with semaphore:
        for protocol in ["http", "https"]:
            ...
            try:
                async with session.get(url, timeout=DEFAULT_TIMEOUT, allow_redirects=False) as response:
...
def main(inputs):
    banner()
    return asyncio.run(main_async(inputs))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ah-Puch - Advanced Subdomain Takeover Detection Tool")
    parser.add_argument(
        'inputs',
        metavar='INPUT',
        type=str,
