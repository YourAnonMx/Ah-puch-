...
def banner():
    ...
    =============================================
           Ah-Puch - TLS Security Configuration Analyzer
    =============================================
    """)
...
def get_tls_security_config(domain: str, ip: str = None, port: int = 443, timeout: int = DEFAULT_TIMEOUT):
    results = []
    ...
    try:
        ...
        with socket.create_connection((target, port), timeout=timeout) as sock:
            with context.wrap_socket(sock, server_hostname=domain) as tls_sock:
                ...
                supported_tls_versions = get_supported_tls_versions(domain, ip, port, timeout)
                supported_ciphers = get_supported_ciphers(domain, ip, port, timeout)
                results.append({
...
def get_supported_tls_versions(domain: str, ip: str, port: int, timeout: int = DEFAULT_TIMEOUT):
    tls_versions = {
    ...
    for name, version in tls_versions.items():
        try:
            ...
            with socket.create_connection((ip if ip else domain, port), timeout=timeout) as sock:
                with context.wrap_socket(sock, server_hostname=domain) as tls_sock:
...
def get_supported_ciphers(domain: str, ip: str, port: int, timeout: int = DEFAULT_TIMEOUT):
    context = ssl.create_default_context()
    context.set_ciphers('ALL:@SECLEVEL=0')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_cipher = {
            executor.submit(test_cipher, domain, ip, port, cipher, timeout): cipher for cipher in cipher_names
        }
...
def test_cipher(domain: str, ip: str, port: int, cipher: str, timeout: int = DEFAULT_TIMEOUT):
    try:
        context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
...
def analyze_tls_security(config):
    ...
    if cert_info.get('Valid To'):
        ...
        days_left = (valid_to - utc_now).days
        if days_left < 30:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Ah-Puch - TLS Security Configuration Analyzer')
    ...
    args.timeout = max(1, min(args.timeout, MAX_TIMEOUT))
    args.threads = max(1, min(args.threads, MAX_WORKERS))
    args.port = max(1, min(args.port, 65535))
    ...
    if not targets:
        console.print(Fore.RED + "[!] No valid target provided.")
        return 2

    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
        future_to_target = {
            executor.submit(get_tls_security_config, domain, ip, args.port, args.timeout): (domain, ip)
