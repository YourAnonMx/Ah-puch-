...
def perform_whois_lookup(domain):
    ...
    if not str(domain or "").strip():
        console.print(f"{Fore.YELLOW}[!] No domain provided for WHOIS lookup.{Style.RESET_ALL}")
        return None
    try:
        console.print(f"{Fore.CYAN}[*] Performing WHOIS lookup for domain: {domain}{Style.RESET_ALL}")
        result = subprocess.run(["whois", domain], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=WHOIS_TIMEOUT)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        try:
            ...
            sys.exit(0)  # Exit with code 0 to prevent errors in ahpuch.py
        except Exception as e:
            console.print(f"{Fore.RED}[!] An unexpected error occurred: {e}{Style.RESET_ALL}")
