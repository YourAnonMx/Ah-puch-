...
=============================================
     Ah-Puch - Shodan Exposure Analyzer
=============================================
""")
...
async def main_async(inputs):
    ...
    if not SHODAN_API_KEY:
        console.print(Fore.RED + "[!] Shodan API key is unavailable; configure it outside the module source.")
        return {"status": "api-key-unavailable", "results": []}
    ...
    return {"status": "success", "results": recon_results}

def main(inputs):
    try:
        result = asyncio.run(main_async(inputs))
        return 1 if result and result.get("status") == "api-key-unavailable" else 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ah-Puch - Shodan Exposure Analyzer")
    parser.add_argument('inputs', nargs='+', help='IP addresses or domains to analyze')
    args = parser.parse_args()
    raise SystemExit(main(args.inputs))
