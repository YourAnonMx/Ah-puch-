...
def banner():
    ...
    =============================================
               Ah-Puch - Advanced SSL Expiry Check
    =============================================
    """)
...
def split_host_port(value: str) -> tuple[str, int]:
    clean = clean_domain_input(value)
    if clean.startswith("[") and "]" in clean:
        host, _, raw_port = clean[1:].partition("]")
        return host, int(raw_port.lstrip(":") or 443)
    if clean.count(":") == 1:
...
def ssl_expiry_check(domain: str, port: int = 443) -> tuple:
    try:
...
def ssl_expiry(domain: str):
    ...
    if not validate_domain(host) or not 1 <= port <= 65535:
    ...
    expiry_date, days_left, cert = ssl_expiry_check(host, port)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ah-Puch - Advanced SSL Expiry Check')
